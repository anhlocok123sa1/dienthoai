<div id="tintuc">
	...
	$kq = $dbh->prepare($sql);
	$kq->bindParam(':from', $from, PDO::PARAM_INT);
	$kq->bindParam(':max_results', $max_results, PDO::PARAM_INT);
	$kq->execute();
		...
		<div class="tintuccon">
			<div class="tintuccon-in">
				<div class="tieudetintuc">
					<p><a href="index.php?content=chitiettintuc&matt=<?php echo $row['matt'] ?>">
							<?php echo $row['tieude'] ?>
						</a></p>
					<span>Ngày đăng tin:
						<?php echo $row['ngaydangtin'] ?>
					</span>
				</div>
				<div class="imgtintuc">
				...
				<div class="noidungtintuc">
					...
					<p class="xemthem"><a href="index.php?content=chitiettintuc&matt=<?php echo $row['matt'] ?>">Xem thêm
							>></a></p>
				</div>
			</div>
		</div>
...
<div id="phantrang_sp">
	...
	$sql = "SELECT COUNT(*) as Num FROM tintuc";
	$query = $dbh->query($sql);
