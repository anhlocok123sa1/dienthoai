            ...
            while ($row = $rows->fetch(PDO::FETCH_ASSOC)) {
                ...
                <form action="index.php?content=cart&action=update&idsp=<?php echo $row['idsp'] ?>" method="POST" name="update">
                    <tr class="sanphamcart">
                        <td>
                            <p class="carta"><a href="index.php?content=chitietsp&idsp=<?php echo $row['idsp'] ?>">
                                    <?php echo $row['tensp'] ?>
                                </a></p>
                        </td>
                        <td>
                        ...
                        <td><input type="text" name="sl" value="<?php echo $_SESSION['cart'][$row['idsp']] ?>"
                                style="width:30px;" /></td>
                        <td>
                            <?php echo number_format(($row['gia'] * ((100 - $row['khuyenmai1']) / 100)) * $_SESSION['cart'][$row['idsp']], 0, ",", ".") ?>
                        </td>
                        <td>
