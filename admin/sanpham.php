...
<table>
	...
	$sql = "SELECT * FROM sanpham INNER JOIN danhmuc ON sanpham.madm = danhmuc.madm ORDER BY idsp DESC LIMIT :from, :max_results";
	...
	if ($dem > 0)
		while ($bien = $query->fetch(PDO::FETCH_ASSOC)) {
			...
			<tr class='noidung_hienthi_sp'>
				<td class="masp_hienthi_sp"><input type="checkbox" name="id[]" class="item" class="checkbox"
						value="<?= $bien['idsp'] ?>" /></td>
				...
				<td class="sl_hienthi_sp">
					<?php echo $bien['soluong'] ?>
				</td>
				<td class="sl_hienthi_sp">
					<?php echo $bien['daban'] ?>
				</td>
				<td class="gia_hienthi_sp">
					<?php echo number_format($bien['gia']) . ' VNÐ' ?>
				</td>
...
<div id="phantrang_sp">
	...
	$sql = "SELECT COUNT(*) as Num FROM sanpham";
	$query = $dbh->query($sql);
