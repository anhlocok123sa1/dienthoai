...
if (isset($_GET['content'])) {
	...
	if ($total > 0) {
		...
		<div class="sanpham">
			...
			$query1 = $dbh->prepare("select tendm from danhmuc where madm= :madm");
			$query1->bindParam(':madm', $_GET['madm']);
			$query1->execute();
			...
			<div class="sanphamcon">
				...
				while ($result = $query->fetch(PDO::FETCH_ASSOC)) { ?>
					...
					<div class="dienthoai">
						...
						<h4>
							<?php echo number_format(($result['gia'] * ((100 - $result['khuyenmai1']) / 100)), 0, ",", "."); ?>
						</h4>
						<div class="button">
							<ul>
								<li>
									<h1><a href="index.php?content=chitietsp&idsp=<?php echo $result['idsp'] ?>"
		...
		<div class="phantrang">
			...
			// $total_results = mysql_result($dbh->query("SELECT COUNT(*) as Num FROM sanpham where madm='{$_GET['madm']}'"), 0);
			$query = $dbh->prepare("SELECT COUNT(*) as Num FROM sanpham WHERE madm = :madm");
			$query->bindParam(':madm', $_GET['madm']);
			$query->execute();
			$result = $query->fetch(PDO::FETCH_ASSOC);
			$total_results = $result['Num'];
